...
const MatplotlibSupplyCurveDashboard: React.FC = () => {
  ...
  const regenerateChart = async (date: string, hour: string, minute: string, scenario: string) => {
    ...
    try {
      console.log(`🔄 Regenerating matplotlib chart for ${date} ${hour}:${minute} (${scenario})`);
      
      // Call the API to regenerate the chart
      const response = await fetch('/api/generate-matplotlib-chart', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Chart Section - Matplotlib Image */}
        <div className="lg:col-span-3 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          {chartLoading && (
            <div className="text-center py-10">
              ...
              <p className="text-gray-900">Generating supply curve chart...</p>
              <p className="text-sm text-gray-500 mt-2">Processing ERCOT market data with Python matplotlib</p>
            </div>
          )}
          ...
          {!chartLoading && !chartError && (
            <div className="relative bg-white rounded-lg overflow-hidden">
              <img
                src={`/supply_curve_matplotlib.svg?t=${imageTimestamp}`}
                alt="Supply Curve Chart"
          ...
          <div className="mt-2 text-xs text-gray-500 text-center">
            Supply: Python matplotlib visualization • Data: {stats.totalDemand.toLocaleString()} MW demand • 2,727 generator segments
          </div>
        </div>
...
export default MatplotlibSupplyCurveDashboard; 
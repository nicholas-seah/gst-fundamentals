...
const InteractiveSupplyCurveDashboard: React.FC = () => {
  ...
  // Statistics based on the actual CSV data processing
  const stats = useMemo(() => ({
    totalDemand: 101989,  // 75% of 135,986 MW
    clearingPrice: 25.50,
    gridUtilization: 75,
    renewableMix: 45.2,
    avgGenCost: 32.75,
  ...
  const regenerateChart = async (date: string, hour: string, minute: string, scenario: string) => {
    ...
    try {
      console.log(`🔄 Regenerating interactive chart for ${date} ${hour}:${minute} (${scenario})`);
      
      // Call the API to regenerate the interactive chart
      const response = await fetch('/api/generate-interactive-chart', {
        method: 'POST',
        headers: {
      ...
      const iframe = document.getElementById('interactive-chart-iframe') as HTMLIFrameElement;
      if (iframe) {
        iframe.src = `/supply_curve_interactive.html?t=${Date.now()}`;
      }
      
      console.log('✅ Interactive chart regenerated successfully');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          {chartLoading && (
            <div className="text-center py-10">
              ...
              <p className="text-gray-900">Generating interactive supply curve chart...</p>
              <p className="text-sm text-gray-600 mt-2">Processing ERCOT market data with Python Plotly</p>
            </div>
          )}
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  ...
                  <p className="text-xs text-red-300 mt-1">
                    Make sure Python and Plotly are installed.
                  </p>
                </div>
              </div>
            </div>
          ...
          {!chartLoading && !chartError && (
            <div className="relative bg-white rounded-lg overflow-hidden border border-gray-300" style={{ height: '650px' }}>
              <iframe
                id="interactive-chart-iframe"
                src={`/supply_curve_interactive.html?t=${Date.now()}`}
                className="w-full h-full border-none rounded"
                onLoad={handleIframeLoad}
                onError={() => setChartError('Failed to load interactive chart')}
                title="Interactive Supply Curve Chart"
          ...
          <div className="mt-2 text-xs text-gray-500 text-center">
            Supply: Interactive Python Plotly • Data: {stats.totalDemand.toLocaleString()} MW demand • 2,727 generator segments • 
            <span className="text-indigo-600">Hover, zoom, and pan enabled</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-1 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="space-y-6">
            ...
            <div>
              <h4 className="text-sm font-semibold text-gray-900 mb-3">Interactive Features</h4>
              <div className="space-y-1 text-xs text-gray-600">
                <div>• Hover for generator details</div>
                <div>• Zoom to focus on price ranges</div>
                <div>• Pan to explore different MW ranges</div>
                <div>• Click legend to hide/show resource types</div>
                <div>• Download chart as PNG</div>
              </div>
            </div>
          </div>
        </div>
...
export default InteractiveSupplyCurveDashboard; 